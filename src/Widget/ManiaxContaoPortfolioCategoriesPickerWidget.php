...
namespace Maniax\ContaoPortfolio\Widget;
...
class ManiaxContaoPortfolioCategoriesPickerWidget extends Widget
{
    /**
     * Submit user input.
     *
    ...
    public function generate()
    {
        ...
        if (!empty($this->varValue) && null !== ($categories = TlManiaxContaoPortfolioCategory::findMultipleByIds((array) $this->varValue))) {
  ...
  <div class="selector_container">'.(($this->sorting && count($values) > 1) ? '
    ...
    <ul id="sort_'.$this->strId.'" class="'.($this->sorting ? 'sortable' : '').'">';
        ...
        if (!$pickerBuilder->supportsContext('maniaxContaoPortfolioCategories')) {
            $return .= '
	<p><button class="tl_submit" disabled>'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</button></p>';
            ...
            $return .= '
	<p><a href="'.ampersand($pickerBuilder->getUrl('maniaxContaoPortfolioCategories', $extras)).'" class="tl_submit" id="pt_'.$this->strName.'">'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</a></p>
	<script>
	  $("pt_'.$this->strName.'").addEvent("click", function(e) {
		...
		Backend.openModalSelector({
		  ...
		  "callback": function(table, value) {
			...
			}).post({"action":"reloadManiaxContaoPortfolioCategories", "name":"'.$this->strId.'", "value":value.join("\t"), "REQUEST_TOKEN":"'.REQUEST_TOKEN.'"});
		  }
		});
	  });
	</script>'.($this->sorting ? '
    ...
    protected function checkValue($input)
    {
        ...
        if (count(array_diff($ids, array_merge($this->rootNodes, Database::getInstance()->getChildRecords($this->rootNodes, 'tl_maniax_contao_portfolio_category')))) > 0) {
